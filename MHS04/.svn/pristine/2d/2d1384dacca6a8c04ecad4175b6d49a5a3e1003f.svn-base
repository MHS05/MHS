...
for(CategoryVO vo : list)
{
	...
	%>		
		...
		<td width="30px" align="right" valign="top">
			<input type="checkbox" name="delNo" id="delNo" value="<%= vo.getPno() %>" onclick='getCheckedCnt()' style="width: 20px; height: 20px">
		</td>
		<td align="left">
			...
			<a href="admininfo.jsp?pno=<%= vo.getPno() %>"><img src="categoryimagedown.jsp?pno=<%= vo.getPno() %>" width="250px" height="300px"><br>
			<%= vo.getPname() %></a><br>
			</div>
		</td>		
	<%
